...
\begin{equation*}
    n(T)=n_i(T)+\sum_{i=1}^{i=3}N_{di}[1-f_{FD}(T,E_{di})]-N_a
\end{equation*}
where
...
\noindent
$f_{FD}(T,E_i)$ is the Fermi--Dirac distribution function for donors
\begin{equation*}
    f_{FD}(T,E_i)=\left\{1+\frac{1}{2}\exp\left[\frac{E_F(T)-E_i}{kT}\right]\right\}^{-1}\,.
\end{equation*}
...
\begin{equation*}
    n(T)=n_i(T)+N_{d}[1-f_{FD}(T,E_{d})]-\sum_{j=1}^{j=2}N_{tj}f_{FD}(T,E_{tj})-N_a
\end{equation*}
where
$N_a$ is the acceptor density,
$n_i$ is the intrinsic concentration
\begin{equation*}
    n_i(T)=[N_c(T)N_v(T)]^{0.5}\exp\left[-\frac{E_G(T)}{2kT}\right]\,,
\end{equation*}
$N_{d}$ is the donor density,
$E_{d}$ is the donor energy level from $E_c$,
